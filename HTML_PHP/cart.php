...
try {
    ...
    if ($method === 'GET') {
        ...
        $items = $db->fetchAll(
            "SELECT ci.id as cart_item_id, ci.quantity, ci.added_at,
                    ...
                    pv.id as variant_id, pv.label as variant_title, 
                    (p.base_price + pv.price_adjustment) as price,
                    (p.base_price + pv.price_adjustment) as unit_price,
                    (p.base_price + pv.price_adjustment) * ci.quantity as line_total,
    ...
    elseif ($method === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
        ...
        if ($variant_id === null || $variant_id === 0) {
            $variant = $db->fetchOne(
                "SELECT pv.id, pv.stock_quantity as stock, p.id as product_id FROM product_variants pv 
                 JOIN products p ON pv.product_id = p.id 
                 WHERE p.id = ? AND p.is_active = 1 AND pv.stock_quantity > 0
    ...
    elseif ($method === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update') {
        ...
        $cart_item = $db->fetchOne(
            "SELECT ci.*, c.user_id, pv.stock_quantity as variant_stock
             FROM cart_items ci
