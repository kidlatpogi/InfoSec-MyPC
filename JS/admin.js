...
function renderUserRows(users, tbody) {
  users.forEach((user) => {
    ...
    // Note: Add unlock button - will be shown for locked accounts via separate check
    const actionButtons = isArchived
      ...
      : `<button class="btn btn-sm" data-action="edit" data-user-id="${user.id}">Edit</button>
         <button class="btn btn-sm btn-danger" data-action="delete" data-user-id="${user.id}">Deactivate</button>
         <button class="btn btn-sm" style="background:#f59e0b;display:none;" id="unlock-user-${user.id}" data-action="unlock" data-user-email="${user.email}">🔓 Unlock</button>`;
    ...
    // Check if account is locked and show unlock button
    if (!isArchived) {
      checkAccountLockStatus(user.email, "user", user.id);
    }
  });
}
...
async function verifyPassword() {
  ...
  return new Promise((resolve) => {
    ...
    console.log("Password modal created:", passwordModal);
...
function initEventDelegation() {
  console.log("Initializing event delegation for CSP compliance");

  // Use event delegation on the main admin container
  ...
  adminContainer.addEventListener("click", (e) => {
    ...
    // Unlock action (uses email)
    else if (button.dataset.userEmail && action === "unlock") {
      unlockUser(button.dataset.userEmail);
    }
    // Employee actions
...
async function loadUsers(includeArchived = null) {
  ...
  try {
    ...
    paginationState.users.originalData = JSON.parse(
...
async function loadEmployees(includeArchived = null) {
  ...
  try {
    ...
    paginationState.employees.totalItems =
      paginationState.employees.allData.length;
...
async function loadProducts() {
  ...
  try {
    ...
    paginationState.products.totalItems =
      paginationState.products.allData.length;
    paginationState.products.currentPage = 1;

    console.log("Products loaded:", paginationState.products.totalItems);
    ...
    .then((data) => {
      ...
      viewContent.innerHTML = `
          ...
          : '<div style="background: #f0f0f0; padding: 3rem; text-align: center; border-radius: 8px; margin-bottom: 1rem;">No Image</div>'
                        ...
                        <div style="margin-bottom: 1rem;">
                            <strong>Category:</strong> ${product.category_name || product.category || "N/A"
        }
                        </div>
                        <div style="margin-bottom: 1rem;">
      ...
      viewTitle.textContent = `Product Details: ${product.name || product.title
        }`;
  ...
  verifyPassword().then((verified) => {
    ...
    ProductsAPI.getProduct(productId)
      .then((data) => {
        ...
        // Set modal title
        modalTitle.textContent = `Edit Product: ${product.name || product.title
          }`;
...
function editUser(userId) {
  ...
  verifyPassword()
    .then((verified) => {
      ...
      console.log("Modal elements:", { userModal, userForm, modalTitle });
...
// Check if account is locked and show unlock button
async function checkAccountLockStatus(email, accountType, userId) {
  try {
    const response = await fetch("/HTML_PHP/admin_auth.php", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({ action: "checkLockout", email: email }),
      credentials: "include",
    });
    ...
    if (data.success && data.locked) {
      // Show unlock button
      const unlockBtn = document.getElementById(`unlock-user-${userId}`);
      if (unlockBtn) {
...
async function unlockUser(email) {
  ...
  showConfirmDialog(
    `Are you sure you want to unlock the account for ${email}? This will clear all failed login attempts.`,
    async () => {
      try {
        await ManagementAPI.unlockAccount(email, "user");
...
function initAdminPage() {
  ...
  console.log("Window functions available:", {
    ...
    reactivateUser: typeof window.reactivateUser,
    unlockUser: typeof window.unlockUser,
  });
  ...
  console.log("initAdminPage completed");
}
...
window.checkAccountLockStatus = checkAccountLockStatus;
