...
function updatePaginationDisplay(tableType) {
  ...
  if (nextBtn) {
    ...
    nextBtn.style.cursor =
      state.currentPage >= maxPages ? "not-allowed" : "pointer";
  }
}
...
function renderUserRows(users, tbody) {
  users.forEach((user) => {
    ...
    const actionButtons = isArchived
      ? `<button class="btn btn-sm btn-success" data-action="reactivate" data-user-id="${user.id}">Reactivate</button>`
      : `<button class="btn btn-sm" data-action="edit" data-user-id="${user.id}" data-user-email="${user.email}">Edit</button>
         <button class="btn btn-sm btn-danger" data-action="delete" data-user-id="${user.id}">Deactivate</button>`;
...
function renderEmployeeRows(employees, tbody) {
  employees.forEach((emp) => {
    ...
    const actionButtons = isArchived
      ? `<button class="btn btn-sm btn-success" data-action="reactivate" data-emp-id="${emp.id}">Reactivate</button>`
      : `<button class="btn btn-sm" data-action="edit" data-emp-id="${emp.id}" data-emp-email="${emp.email}">Edit</button>
         <button class="btn btn-sm btn-danger" data-action="delete" data-emp-id="${emp.id}">Deactivate</button>`;
...
async function verifyPassword() {
  return new Promise((resolve) => {
    ...
    verifyBtn.addEventListener("click", async () => {
      ...
      const apiPath =
        window.router && window.router.baseRoot
...
function initEventDelegation() {
  ...
  superadminContainer.addEventListener("click", (e) => {
    ...
    // Admin actions
    if (button.dataset.adminId) {
      const adminId = parseInt(button.dataset.adminId);
      if (action === "edit") {
    ...
    // User actions
      ...
      if (action === "edit") {
        ...
        editUser(userId, email);
      } else if (action === "delete") deleteUser(userId);
      else if (action === "reactivate") reactivateUser(userId);
    }
    // Employee actions
    else if (button.dataset.empId) {
      const empId = parseInt(button.dataset.empId);
      if (action === "edit") {
        const email = button.dataset.empEmail;
        editEmployee(empId, email);
      } else if (action === "delete") deleteEmployee(empId);
      else if (action === "reactivate") reactivateEmployee(empId);
    }
    // Product actions
    ...
    else if (button.dataset.accountEmail && action === "unlock") {
      const email = button.dataset.accountEmail;
      const accountType = button.dataset.accountType;
      unlockAccount(email, accountType);
    }
    // Variant remove action
    else if (button.dataset.variantIdx && action === "remove-variant") {
      const idx = parseInt(button.dataset.variantIdx);
      removeVariant(idx);
    }
  });

  // Add change event delegation for variant inputs
  superadminContainer.addEventListener("change", (e) => {
    const input = e.target.closest("input[data-variant-idx]");
    if (!input) return;
    ...
    console.log("Variant updated:", idx, field, value);
...
function filterUsersTable() {
  ...
  if (query === "") {
    ...
    paginationState.users.allData = paginationState.users.originalData.filter(
      (user) => {
        const email = user.email.toLowerCase();
        const name = `${user.first_name} ${user.last_name}`.toLowerCase();
        const status = (
          user.is_archived == 1 ? "deactivated" : "active"
        ).toLowerCase();
...
function filterTable(tableBodyId) {
  ...
  if (query === "") {
    // Reset to all data if search is cleared
    paginationState[tableType].allData =
      paginationState[tableType].originalData;
  } else {
    // Filter original data
    paginationState[tableType].allData = paginationState[
      tableType
  ...
  paginationState[tableType].totalItems =
    paginationState[tableType].allData.length;
...
async function loadAdmins() {
  ...
  try {
    ...
    paginationState.admins.originalData = JSON.parse(
...
async function loadEmployees(includeArchived = null) {
  ...
  try {
    ...
    paginationState.employees.originalData = JSON.parse(
...
function populateOrderStatusYearSelector() {
  ...
  if (
    paginationState.orders.allData &&
    paginationState.orders.allData.length > 0
  ) {
...
async function editStock(productId) {
  ...
  try {
    ...
    if (variants.length === 0) {
      variantsContainer.innerHTML =
        '<p style="color: #666;">No variants found for this product</p>';
    } else {
      variants.forEach((variant, index) => {
        const variantDiv = document.createElement("div");
        variantDiv.className = "stock-variant-item";
        variantDiv.style.cssText =
          "margin-bottom: 1rem; padding: 1rem; border: 1px solid #ddd; border-radius: 8px;";
      ...
      document.querySelectorAll(".btn-stock-increase").forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          const input = document.querySelector(
            `.variant-stock-input[data-variant-id="${variantId}"]`,
          );
  ...
  try {
    ...
    const itemsHTML = order.items
      .map((item) => {
        ...
        const baseRoot =
          window.router && window.router.baseRoot ? window.router.baseRoot : "";
    ...
    setTimeout(() => {
      ...
      if (updateBtn && statusSelect) {
        updateBtn.addEventListener("click", async () => {
          ...
          if (
            !confirm(
              `Are you sure you want to change the order status to "${statusOptions.find((s) => s.value === newStatus).label}"?`,
            )
          ) {
            return;
          }
...
function renderVariantsEditor() {
  ...
  if (currentVariants.length === 0) {
    container.innerHTML =
      '<p style="text-align: center; color: #999; padding: 2rem;">No variants yet. Click "+ Add Variant" to create one.</p>';
  } else {
    container.innerHTML = currentVariants
      .map(
        (variant, idx) => `
...
function removeVariant(idx) {
  if (
    confirm(`Remove variant "${currentVariants[idx]?.title || "Variant"}"?`)
  ) {
...
// ========================================
// PAGINATION BUTTON LISTENERS (CSP COMPLIANT)
// ========================================

function setupPaginationListeners() {
  // Admins pagination
  document
    .getElementById("admins-prev-btn")
    ?.addEventListener("click", () => prevPage("admins"));
  document
    .getElementById("admins-next-btn")
    ?.addEventListener("click", () => nextPage("admins"));

  // Users pagination
  document
    .getElementById("users-prev-btn")
    ?.addEventListener("click", () => prevPage("users"));
  document
    .getElementById("users-next-btn")
    ?.addEventListener("click", () => nextPage("users"));

  // Employees pagination
  document
    .getElementById("employees-prev-btn")
    ?.addEventListener("click", () => prevPage("employees"));
  document
    .getElementById("employees-next-btn")
    ?.addEventListener("click", () => nextPage("employees"));

  // Products pagination
  document
    .getElementById("products-prev-btn")
    ?.addEventListener("click", () => prevPage("products"));
  document
    .getElementById("products-next-btn")
    ?.addEventListener("click", () => nextPage("products"));

  // Orders pagination
  document
    .getElementById("orders-prev-btn")
    ?.addEventListener("click", () => prevPage("orders"));
  document
    .getElementById("orders-next-btn")
    ?.addEventListener("click", () => nextPage("orders"));

  // Audit pagination
  document
    .getElementById("audit-prev-btn")
    ?.addEventListener("click", () => prevAuditPage());
  document
    .getElementById("audit-next-btn")
    ?.addEventListener("click", () => nextAuditPage());
...
function initModals() {
  ...
  // Setup pagination button listeners (CSP compliant)
...
async function loadAuditLogs(page = 1) {
  ...
  tbody.innerHTML =
    '<tr><td colspan="7" style="text-align:center;padding:2rem;">Loading audit logs...</td></tr>';

  try {
    const data = await AuditAPI.getAuditTrail(
      page,
      auditPaginationState.perPage,
      filters,
    );
    ...
    tbody.innerHTML =
      '<tr><td colspan="7" style="text-align:center;padding:2rem;color:#ef4444;">Error loading audit logs</td></tr>';
  }
}
...
async function showLoginHistory() {
  ...
  try {
    const typeFilter =
      document.getElementById("login-history-type-filter")?.value || "";
    const data = await AuditAPI.getLoginHistory(1, 50, typeFilter || null);

    if (!data.login_history || data.login_history.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="text-align:center;padding:2rem;color:#666;">No login history found</td></tr>';
      return;
    }
...
async function showLockedAccounts() {
  ...
  try {
    ...
    if (!data.locked_accounts || data.locked_accounts.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" style="text-align:center;padding:2rem;color:#22c55e;">No locked accounts</td></tr>';
      return;
    }
    ...
    data.locked_accounts.forEach((account) => {
      ...
      row.innerHTML = `
        ...
        <td><button class="btn btn-sm" style="background:#f59e0b;color:white;" data-action="unlock" data-account-email="${account.email}" data-account-type="${account.account_type}">Unlock</button></td>
      `;
      tbody.appendChild(row);
    });
  } catch (error) {
    ...
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align:center;padding:2rem;color:#ef4444;">Error loading data</td></tr>';
  }
}
...
async function loadSalesDashboard(
  ...
  try {
    ...
    const errorMsg =
      error && error.message
        ? error.message
        : "Unknown error - please try logging in again";
    if (topProductsList) {
      topProductsList.innerHTML =
        '<p style="color: #ef4444; text-align: center; padding: 1rem;">Error loading analytics: ' +
        errorMsg +
        "</p>";
    }
  }
...
function renderSalesChart(period) {
  ...
  switch (period) {
    case "daily":
      renderDailyChart(
        currentAnalyticsData.daily,
        currentAnalyticsData.selected_year,
        currentAnalyticsData.selected_month,
      );
      break;
    case "weekly":
      renderWeeklyChart(
...
function loadChartJS() {
  return new Promise((resolve, reject) => {
    ...
    script.src = window.router
      ? window.router.baseRoot + "/JS/chart.min.js"
      : "/JS/chart.min.js";
...
function updateSalesSummary(data) {
  ...
  if (todaySales)
    todaySales.textContent = formatPHP(data.today?.total_sales || 0);
  if (todayOrders)
    todayOrders.textContent = `${data.today?.order_count || 0} orders`;

  // This week's sales
  const weekSales = document.getElementById("week-sales");
  const weekOrders = document.getElementById("week-orders");
  if (weekSales)
    weekSales.textContent = formatPHP(data.this_week?.total_sales || 0);
  if (weekOrders)
    weekOrders.textContent = `${data.this_week?.order_count || 0} orders`;

  // This month's sales
  const monthSales = document.getElementById("month-sales");
  const monthOrders = document.getElementById("month-orders");
  if (monthSales)
    monthSales.textContent = formatPHP(data.this_month?.total_sales || 0);
  if (monthOrders)
    monthOrders.textContent = `${data.this_month?.order_count || 0} orders`;

  // This year's sales
  const yearSales = document.getElementById("year-sales");
  const yearOrders = document.getElementById("year-orders");
  if (yearSales)
    yearSales.textContent = formatPHP(data.this_year?.total_sales || 0);
  if (yearOrders)
    yearOrders.textContent = `${data.this_year?.order_count || 0} orders`;

  // Completed orders
  const completedSales = document.getElementById("completed-sales");
  const completedOrders = document.getElementById("completed-orders");
  if (completedSales)
    completedSales.textContent = formatPHP(
      data.completed_orders?.total_sales || 0,
    );
  if (completedOrders)
    completedOrders.textContent = `${data.completed_orders?.order_count || 0} orders`;
}
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            ...
            label: (ctx) =>
              ctx.dataset.label === "Sales (₱)"
                ? `Sales: ${formatPHP(ctx.raw)}`
                : `Orders: ${ctx.raw}`,
          },
        },
      },
...
function renderWeeklyChart(weeklyData, year) {
  ...
  const currentWeek = Math.ceil(
    (new Date() - new Date(year, 0, 1)) / (7 * 24 * 60 * 60 * 1000),
  );
  const maxWeek =
    year === new Date().getFullYear() ? Math.min(currentWeek + 1, 52) : 52;
        ...
        tooltip: {
          callbacks: {
            label: function (context) {
              ...
              return [`Sales: ${formatPHP(context.raw)}`, `Orders: ${orders}`];
            },
          },
        },
      },
...
function initProfileHandlers() {
  ...
  const loginHistoryTypeFilter = document.getElementById(
    "login-history-type-filter",
  );
  ...
  if (lockedAccountsModal) {
    ...
    if (closeBtn) {
      closeBtn.addEventListener("click", () =>
        lockedAccountsModal.classList.remove("open"),
      );
    }
    lockedAccountsModal.addEventListener("click", (e) => {
      if (e.target === lockedAccountsModal)
        lockedAccountsModal.classList.remove("open");
    });
  }
}
...
function initSuperadminPage() {
  ...
  initSuperadminTabs();
  initEventDelegation(); // CSP-compliant button handlers
  initModals();
  initLogout();
  initProfileHandlers();
}
